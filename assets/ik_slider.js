;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_slider',
		defaults = {
			'instructions': 'Use the left/down and right/up arrow keys to change the value by one increment, home and end keys to set value to minimum and maximum values correspondingly.',
			'minValue': 0,
			'maxValue': 100,
			'nowValue': 0,
			'step': 1
		...
		if( !plugin.textfield.is(':text') ) {
			...
			plugin.knob = $('<div/>')
				.attr({
					...
					'aria-valuenow': plugin.options.minValue, // set silder current value
