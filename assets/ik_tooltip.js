		...
		if(tip.length > 0) {
			...
			$tooltip = $('<span/>') // create tooltip
				...
				.attr({
					...
					'aria-hidden': 'true', // hide it from screen reader to prevent it from being read twice
					'aria-live': 'polite' // make it a live region
				});
			...
			$elem
				...
				.on('blur', function(event) {
					if (!$tooltip.hasClass('mouseover')) { // hide tooltip if mouse not over the current element
						$tooltip
							.attr({
								'aria-hidden': 'true'
							})
