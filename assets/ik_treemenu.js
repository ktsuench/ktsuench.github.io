;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
			...
			.each(function(i, el) {
				...
				if ($me.children('ul').length) {  // if the current treeitem has submenu
					...
					if (plugin.options.expandAll) { // expand or collapse all tree levels based on configuration
						...
						$me.attr({'aria-expanded': false}).addClass('collapsed');
					}
					
					$me.attr({'aria-label': $me.children('span:first').text()})
					.children('span').addClass('folder').attr({'role': 'presentation'});
				} else {
		...
		$elem.find('li:first').attr({'tabindex': 0});
	};
	...
	Plugin.prototype.selectItem = function($item, plugin) {
		var $elem = plugin.element;
		 ...
		 $elem.find('[aria-selected=true]').attr({'tabindex': -1, 'aria-selected': false});
		
		$elem.find('.focused') // remove highlight form previousely selected treeitem
			.removeClass('focused');
		
		$elem.find('li').attr({'tabindex': -1});
		$item.attr({'tabindex': 0, 'aria-selected': true});
	...
	Plugin.prototype.toggleSubmenu = function($item) {
		...
		if($item.children('ul').length) { // check if the treeitem contains submenu
			...
			if ($item.hasClass('collapsed')) {  // expand if collapsed
				...
				$item.attr({'aria-expanded': true}).removeClass('collapsed');
        
			} else { 							// otherwise collapse
      
				$item.attr({'aria-expanded': false}).addClass('collapsed');
