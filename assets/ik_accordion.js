	...
	Plugin.prototype.onKeyDown = function (event) {
		...
		switch (event.keyCode) {
			...
			case ik_utils.keys.space:
				...
				break;
			// use up arrow to jump to previous header
			case ik_utils.keys.up:
				ind = plugin.headers.index($header);
				if (ind > 0) plugin.headers.eq(--ind).find('.button').focus();
				break;
			// use down arrow to jump to next header
			case ik_utils.keys.down:
				ind = plugin.headers.index($header);
				if (ind < plugin.headers.length - 1) plugin.headers.eq(++ind).find('.button').focus();
				break;
		}
	};
	...
	Plugin.prototype.togglePanel = function (event) {
		...
		if(plugin.options.autoCollapse) { // expand current panel and collapse the rest
			if($(event.currentTarget).hasClass('expanded')) {
				...
				$hdr = $(event.currentTarget).parent();
				$btn = $hdr.find('.button');

				$btn.removeClass('expanded');
				$hdr.next().slideUp(plugin.options.animationSpeed);
				$notification.text('collapsed heading');
			} else {
				plugin.headers.each(function(i, el) {
					...
					if($btn[0] != $(event.currentTarget)[0]) { 
						...
						$notification.text('expanded heading');
					}
				});
			}
			...
			$notification.text(!isVisible ? 'expanded' : 'collapsed');
